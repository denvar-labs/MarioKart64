...
constant CMapState(0x800DC5A8)   // 0 = progress, 1 = map (verify this address in your ROM)
...
scope Init: {
  ...
  Menu:
    ...
    Default:
      la t0, Options
      ori t1, r0, 0x02
      sb t1, 2 (t0)       // scaling default = 30 fps
      sb t1, 5 (t0)       // trophies skip
      sb t1, 6 (t0)       // mp music enabled
      sb t1, 10 (t0)      // vs tracks all cups
      sb t1, 14 (t0)      // vs scores enabled
      sb t1, 15 (t0)      // polling fix 30hz
      ori   t2, r0, 0x01
      sb    t2, 16(t0)    // bolt fix default = 0x01 (default)
      sb    t2, 17(t0)    // c spam default = 0x01 (default)
      sb    t2, 18(t0)    // pause hold default = 0x01 (default)
  lw ra, 0x14 (sp)
  jr ra
  addiu sp, 0x18
}
...
dd 0x00000002 // C Spam Fix
dd MenuEntry17
dd MenuEntry17Setting1, MenuEntry17Setting2, 0x00000000

dd 0x00000002 // Pause Hold Fix
dd MenuEntry18
dd MenuEntry18Setting1, MenuEntry18Setting2, 0x00000000
...
la t0, MenuStrings
addiu t1, r0, 0x01
addiu a1, r0, 0x28        // Moved up to fit all entries (was 0x38)
MenuArrayLoop:
  jal MenuColorCycle        // Animate the highlight color
  nop
  lw t2, 0x04 (t0)
  beq t2, r0, MenuInput
  nop
...
// ---------------------------------------------------------
// Menu Color Cycling (enhanced visual)
// ---------------------------------------------------------
scope MenuColorCycle: {
  addiu sp, sp, -0x10
  sw    ra, 0x00(sp)
  sw    a0, 0x04(sp)
  sw    a1, 0x08(sp)
  ...
  // Check if the menu is active (simple flag at 0x800F6990)
  la    t0, 0x800F6990
  lw    t0, 0(t0)
  beqz  t0, End

  // Read frame counter and create a slowly changing color index
  la    t0, 0x800DC5E0
  lw    t0, 0(t0)
  srl   t0, t0, 4          // divide by 16 to slow down the cycle
  andi  t0, t0, 0x7        // 8 colors (0-7)
...
ColorPalette:
  .word 0xFFD700FF // Gold
  .word 0x00BFFFFF // Deep sky blue
  .word 0x32CD32FF // Lime green
  .word 0xFF4500FF // Orange red
  .word 0xBA55D3FF // Medium purple
  .word 0xFF1493FF // Deep pink
  .word 0x00FA9AFF // Medium spring green
  .word 0xFF6347FF // Tomato

// ---------------------------------------------------------
// Same Character Sound
// ---------------------------------------------------------
scope SameCharacter: {
  sb s0, 0 (t9)
  beq v0, r0, End
...
// ---------------------------------------------------------
// Character Stats Override (New, no external tables needed)
// ---------------------------------------------------------
scope CharacterStatOverride: {
  addiu sp, sp, -0x18
  sw    ra, 0x14(sp)
  sw    t0, 0x10(sp)
  sw    t1, 0x0C(sp)
  ...
  // Execute the original instruction that we replaced: sb a1, 0x6C(a0)
  sb    a1, 0x6C(a0)

  // Read the stats option (Options+1)
  LuiLb(t0, Options+1)

  // --- All Yoshi (2) ---
  addiu t1, r0, 2
  bne   t0, t1, CheckAllWario
  nop
  li    t1, 2                  // Yoshi character ID
  b     DoOverride
  nop
...
CheckAllWario:
  // --- All Wario (3) ---
  addiu t1, r0, 3
  bne   t0, t1, CheckMoreWarios
  nop
  li    t1, 5                  // Wario character ID
  b     DoOverride
  nop
...
CheckMoreWarios:
  ...
  // Only Yoshi(2), Toad(3), Peach(6), Bowser(7) become Wario(5)
  lbu   t1, 0x6C(a0)          // Original character ID
  addiu t0, r0, 2
  beq   t1, t0, SetWario
  addiu t0, r0, 3
  beq   t1, t0, SetWario
...
DoOverride:
  // Overwrite the stored character ID
  sb    t1, 0x6C(a0)
...
// ---------------------------------------------------------
// C Spam Fix: mute sound + 3s cooldown + auto-revert map
// ---------------------------------------------------------
constant ToggleMapFunc(0x80283670)  // Verify from decomp

scope CSpamSound: {
  LuiLb(t0, Options+17)
  OriBne(t0, 0x02, at, OriginalSound)
...
OriginalSound:
  li    a0, 0x49008000      // SOUND_MENU_SELECT
  ori   a1, r0, 0xFFFF
  jal   play_sound2
  nop
  jr    ra
...
scope CSpamDelay: {
  LuiLb(t0, Options+17)
  OriBne(t0, 0x02, at, OriginalToggle)
  ...
  la    t0, last_c_spam_time
  lw    t1, 0(t0)

  la    t2, 0x800DC5E0       // Global frame counter
  lw    t2, 0(t2)
  subu  t3, t2, t1
  sltiu at, t3, 90           // 90 frames = 3 seconds cooldown
  bnez  at, skip_toggle

  // Allow toggle: update timestamp and call the original toggle function
  sw    t2, 0(t0)
  jal   ToggleMapFunc
  nop
  // After toggling, store the timestamp if map is now active
  la    t0, CMapState
  lbu   t1, 0(t0)             // 0 = progress, 1 = map
  beqz  t1, 1f
  la    t2, map_on_time
  sw    v0, 0(t2)             // Actually we need the frame counter; we'll store it below
1:
  lw    ra, 0x14(sp)
  lw    t1, 0x10(sp)
  lw    t2, 0x0C(sp)
...
skip_toggle:
  ...
  lw    t2, 0x0C(sp)
  addiu sp, sp, 0x18
  li    t0, 0x80283700       // Jump past the original call
  jr    t0
  nop

OriginalToggle:
  // DEFAULT: just execute the original toggle function and return
  addiu sp, sp, -0x18
  sw    ra, 0x14(sp)
  jal   ToggleMapFunc
  nop
...
// Auto-revert: if map is on and C spam enabled, revert after 3 seconds
scope CMapAutoRevert: {
  LuiLb(t0, Options+17)
  OriBne(t0, 0x02, at, End)   // Only active when C spam is enabled

  la    t0, CMapState
  lbu   t0, 0(t0)              // Current state: 0 = progress, 1 = map
  beqz  t0, End                 // Map is off, nothing to do

  la    t0, map_on_time
  lw    t1, 0(t0)              // Timestamp when map was turned on
  la    t2, 0x800DC5E0         // Global frame counter
  lw    t2, 0(t2)
  subu  t2, t2, t1
  sltiu at, t2, 90             // Less than 3 seconds?
  bnez  at, End                 // Still within window, keep map

  // Time's up: force back to progress (0)
  la    t0, CMapState
  sb    r0, 0(t0)              // Set state to 0 (progress)
...
// ---------------------------------------------------------
// Pause Hold Fix (hold START for 3 seconds to pause)
// ---------------------------------------------------------
scope PauseHoldFix: {
  LuiLb(t0, Options+18)
  OriBne(t0, 0x02, at, Original)
  ...
  la    t0, pause_start_time
  lw    t2, 0(t0)

  bnez  t2, CheckHoldTime
  nop
  sw    t1, 0(t0)
  b     SkipPause
  nop
...
// ---------------------------------------------------------
// Continue menu text selection (KA/VA)
// ---------------------------------------------------------
...
// ============================================================
// All external stats files have been removed.
// Character stats are now handled entirely by CharacterStatOverride.
// ============================================================
...
// Character Stats Override (new method)
origin 0x0273138
base 0x802B3138
jal CharacterStatOverride
nop
...
// C Spam: toggle delay + auto-revert timer set
origin 0x126B0C
base 0x802836EC
jal CSpamDelay
nop

// C Spam: auto-revert check (every frame)
origin 0x126AA0
base 0x802836C0
jal CMapAutoRevert
nop

// Pause Hold Fix (START 3 seconds)
origin 0x028F9D0
base 0x8028F9D0
jal PauseHoldFix
nop
